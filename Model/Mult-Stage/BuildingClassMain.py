...
#import Accuracy
#
writer = SummaryWriter('tensorflow/single_buillding_nos1_fold1')
...
fold_idx=np.load("/data/CS/Challenge/GermanAIChallenge2018/Round2/Building_Kfold_idx.npy")
...
trn_val_y=np.load("/data/CS/Challenge/GermanAIChallenge2018/Round2/trn_val_y.npy")
...
train_loader = DataLoader(dataset=train_datasets, batch_size=512, sampler=SubsetRandomSampler(range(train_new.shape[0])),num_workers=3)
valid_loader = DataLoader(dataset=valid_datasets, batch_size=512, num_workers=3)
...
model=torch.load("/data/DW/Challenge/GermanAIChallenge2018/Round_2/SmallClass/single_building_model/ResNet50_optionB_single_Building_nos1_fold1_129.pkl",map_location='cpu')

# if torch.cuda.device_count() > 1:
#     model = nn.DataParallel(model)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(),lr=9e-6, weight_decay=1e-2)
...
batch_idx_trn=41471
batch_idx_val=10401
for epoch in range(131,151):

    batch_idx_trn=train(epoch,batch_idx_trn)
    batch_idx_val,pred_val=valid(epoch,batch_idx_val)

    print('训练batch', batch_idx_trn, '验证batch', batch_idx_val)

    if epoch%3==0:
        path = "/data/DW/Challenge/GermanAIChallenge2018/Round_2/SmallClass/single_building_model/ResNet50_optionB_single_Building_nos1_fold1_"+ str(epoch)+".pkl"
